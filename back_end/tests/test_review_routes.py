...
async def test_post_tour_review(
    test_client: TestClient, engine: AIOEngine, admin_token_header: dict
):
    tours = await engine.find(Tour)
    tour_id = tours[0].id
    review = {"review": "This is my test review about the tour", "rating": 4}
    response = await test_client.post(
        f"/api/v1/tours/{tour_id}/reviews", headers=admin_token_header, json=review
    )
    assert response.status_code == 200
...
async def test_get_tour_review(
    test_client: TestClient, engine: AIOEngine, admin_token_header: dict
):
    tours = await engine.find(Tour)
    tour_id = tours[0].id
    response = await test_client.get(
        f"/api/v1/tours/{tour_id}/reviews", headers=admin_token_header
    )
    assert response.status_code == 200
    assert response.json()["status"] == "success"
...
async def test_get_review_by_id(
    ...
    response = await test_client.get(
        f"/api/v1/reviews/{review_id}", headers=admin_token_header
    )
    assert response.status_code == 200
    assert response.json()["status"] == "success"
