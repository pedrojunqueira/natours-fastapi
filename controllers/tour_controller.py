...
def query_params(query):
    sort_by = query.get("sort").split(",") if query.get("sort") else Tours.createdAt.desc()
    sort_by = tuple([getattr(Tours, atr) for atr in sort_by]) if sort_by else Tours.createdAt.desc()
